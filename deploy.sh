...
gcloud run deploy "$SERVICE" \
  ...
  --cpu 2 \
  --timeout 300 \
  --set-env-vars "GCP_PROJECT_ID=$PROJECT_ID,GCP_REGION=$REGION,GOOGLE_GENAI_USE_VERTEXAI=TRUE,GOOGLE_CLOUD_PROJECT=$PROJECT_ID,GOOGLE_CLOUD_LOCATION=$REGION,GEMINI_TUNED_MODEL=${SFT_ENDPOINT:-projects/$PROJECT_ID/locations/$REGION/endpoints/YOUR_ENDPOINT_ID}" \
  --project="$PROJECT_ID"
